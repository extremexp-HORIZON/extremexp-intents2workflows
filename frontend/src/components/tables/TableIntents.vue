          ...
          <div class="centered-table">
            <table>
              ...
              <tr v-if="props.row.workflows && props.row.workflows.length > 0" v-for="workflow in props.row.workflows" :key="workflow.workflowID">
                <td>{{ workflow.name }}</td>
                <td>
                  <q-btn @click="visualizeWorkflow(workflow.visual_representation)" color="primary" icon="visibility" size="sm"></q-btn>
                </td>
                <td>
                  <q-btn dense round flat color="grey" @click="workflowsStore.downloadWorkflowSchema(workflow)" icon="download" label="RDF"></q-btn>
...
return intentsStore.intents
  ...
  .map((intent) => {
    return {
      ...intent,
      dataProductName: intent.data_product.name, // Get data product name from data product
...
const columns = [
  {name: "intentName", label: "Intent Name", align: "center", field: "name", sortable: true},
...
const deleteIntent = (propsRow) => {
  ...
  confirmDelete = () => {
    intentsStore.deleteIntent(propsRow.row.name)
  }
}
...
const deleteWorkflow = (intent, workflow) => {
  ...
  confirmDelete = () => {
    workflowsStore.deleteWorkflow(intent.name, workflow.name)
  }
}
